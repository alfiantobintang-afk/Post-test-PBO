...
- **Enkapsulasi & Proteksi Data**: Menggunakan atribut *private* (`__status`, `__jumlah_item`, `__password`, `__total_harga`) yang dikelola secara aman melalui `@property` (Getter) dan Setter.
...
- **Class & Static Methods**: Penggunaan `@classmethod` untuk mengubah state tingkat kelas dan `@staticmethod` untuk utilitas independen seperti validasi teks serta format rupiah.
...
* **Fungsi Utama**:
  ...
  * `@property status` & `@status.setter` — Mengambil dan menguji validasi status (`Aktif` / `Nonaktif`).
...
* **Fungsi Utama**:
  ...
  * `@property jumlah_item` & `@jumlah_item.setter` — Mengelola stok/jumlah item dengan proteksi nilai non-negatif.
...
* **Fungsi Utama**:
  ...
  * `@property password` & `@password.setter` — Mengubah *password* dengan syarat minimal 6 karakter.
...
* **Fungsi Utama**:
  ...
  * `@property total_harga` & `@total_harga.setter` — Proteksi manipulasi total harga.
